...
for(i in 1:num.games) {
  ...
  for(j in 1:num.plays){
    ...
    if(homeQ == "0") {
      ...
      if(hitQ == TRUE) {
        # Took out ball.in.playQ and ball.in.playQ == hitQ 
        play.hit.info <- c(i, game.plays[play.num,3], inning, play.portion1.char.cod, pitch)
        #print(play.hit.info)
...
#colnames(season.hit.mat) <- c("game.num","retroID", "inning", "hit.cod", "pitches")
rownames(season.hit.mat) <- NULL
season.hit.mat

season.hit.mat <- data.frame(
  as.numeric(season.hit.mat[,1]),
  season.hit.mat[,2],
  as.numeric(season.hit.mat[,3]),
  season.hit.mat[,4],
  season.hit.mat[,5]
)
colnames(season.hit.mat) <- c("game.num","retroID", "inning", "hit.cod", "pitches")
write.csv(season.hit.mat, "/home/npetraco/latex/papers/baseball_peter_s/HOU.home.season.hits.csv", row.names = F)

season.hit.mat$hit.cod
