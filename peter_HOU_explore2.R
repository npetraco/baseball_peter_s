...
for(j in 1:num.plays){
  ...
  if(homeQ == "0") {
    ...
    if(hitQ == TRUE) {
      play.hit.info <- c(game.plays[play.num,3], inning, play.portion1.char.cod, pitch, ball.in.playQ, ball.in.playQ == hitQ)
      #print(play.hit.info)
...
colnames(game.hit.mat) <- c("retroID", "inning", "hit.cod", "pitches", "ball.in.playQ", "play.hit.consistent?")
game.hit.mat
